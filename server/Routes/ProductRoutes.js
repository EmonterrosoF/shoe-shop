...
import { queryProductSchema } from "../validations/productSchemaValidation.js";

const router = Router();

// obtiene todos los productos o obtiene productos segun la query por nombre
// adicional con paginacion
router.get("/", ValidateData(queryProductSchema), async (req, res) => {
  try {
    const pageSize = 5;
...
router.get("/all", protectedUser, admin, async (req, res) => {
...
// DELETE PRODUCT
router.delete("/:id", protectedUser, admin, async (req, res) => {
  const product = await Product.findById(req.params.id);
  if (product) {
...
// CREATE PRODUCT
router.post("/", protectedUser, admin, async (req, res) => {
...
// UPDATE PRODUCT
router.put("/:id", protectedUser, admin, async (req, res) => {
  const { name, price, description, image, countInStock } = req.body;
  const product = await Product.findById(req.params.id);
  if (product) {
